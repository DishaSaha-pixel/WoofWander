export class Tunes{
    constructor(game){
        ...
        this.prevIndex=this.currentTuneIndex;
            ...
            this.broken = document.getElementById('broken'); // Add broken tune image
            this.broken1 = document.getElementById('broken1');
            this.broken2 = document.getElementById('broken2');

            this.spark=document.getElementById('spark');
            this.wrong_spark=document.getElementById('wrong_spark');
...
//add
            ...
            this.tuneslist=[
                ...
                { x: game.width, y: 550, width: 60, height: 60, img: this.t5, number: 5, collected: false },
                { x: game.width, y: 550, width: 60, height: 60, img: this.t6, number: 6, collected: false },

                            // Add broken tunes
                { x: game.width, y: 600, width: 60, height: 60, img: this.broken2, number: 0, collected: false},
                { x: game.width, y: 600, width: 60, height: 60, img: this.broken, number: 0, collected: false},
                { x: game.width, y: 600, width: 60, height: 60, img: this.broken1, number: 0, collected: false},
                
         //   { x: game.width, y: 450, width: 60, height: 60, img: this.spurious, number: -1, collected: false,  },
    ...
    draw(context){
        ...
        checkTuneCollection(player) {
         ...
         /*   if (!currentTune.collected &&
          ...
          /*  if (currentTune.broken) {
                console.log('Broken tune collected, no sparks or sound.');
                currentTune.collected = true;
                return -1;
            } */
        ...
        drawwrongspark(context){
            // console.log('entered drawwwwwwwwwwwww');
             context.drawImage(this.wrong_spark,0,0,271,186,this.sparkX-25,this.sparkY-25,100,100);  
